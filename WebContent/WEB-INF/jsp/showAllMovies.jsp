...
<body>
 <fmt:setLocale value="${sessionScope.local}" />
    ...
    <fmt:message bundle="${loc}" key="locale.addNewMovie.movie_id" var="movie_id" />
    ...
    <fmt:message bundle="${loc}" key="locale.addNewMovie.movie_list_title" var="movie_list_title" />
	...
	<table cellspacing="2" border="1" cellpadding="5" bordercolor="gray">
		...
		<c:forEach items="${moviesList}" var="movies">
					<c:if test="${count >= allMoviesFirstRow && count < allMoviesLastRow}">
						<tr>
							<td align="center" width="75"><c:out value="${movies.movieID}"></c:out></td>
							<td align="center" width="200"><a href="Servlet?command=concreteMovieInfo&movie_id=${movies.movieID}">${movies.movieTitle}</a></td>
							<td align="center" width="100"><c:out value="${movies.movieDirector}"></c:out></td>
							<td align="center" width="100"><c:out value="${movies.movieReleasedYear}"></c:out></td>
							<td align="center" width="75"><c:out value="${movies.movieAVGRate}"></c:out></td>
						</tr>
					</c:if>
	...
	<form action="Servlet" method="post">
		<c:if test="${count > allMoviesLastRow}">
			<input type="hidden" name="first_row_parameter_name" value="allMoviesFirstRow">
			<input type="hidden" name="last_row_parameter_name" value="allMoviesLastRow">
			<input type="hidden" name="number_of_pages_parameter_name" value="allMoviesNumberOfPages">
			<input type="hidden" name="current_page_number_parameter_name" value="allMoviesCurrentPageNumber">
			<input type="hidden" name="current_page_URL" value="Servlet?command=goToShowAllMoviesPageCommand">
	...
	<h4>
		<c:out value="${page} ${sessionScope.allMoviesCurrentPageNumber} ${from } ${sessionScope.allMoviesNumberOfPages}" />
		</h4>
	<br>
		<c:if test="${!empty sessionScope.user && sessionScope.user.userRole == 'ADMIN'}">
		<form action="Servlet" method="post" >
