...
<body>
	...
	<fmt:message bundle="${loc}" key="locale.registration.wrong_params"	var="registration_wrong_params" />
	...
	<form action="Servlet" method="post">
		...
		<table width="100%" cellspacing="0" cellpadding="4">
			...
			<tr>
				...
				<td>
					<input type="text" size="50" name="last_name" pattern="^([A-Za-zА-Яа-яЁё]){3,44}$" required />
				</td>
			</tr>
			<tr>
				<td align="right" title="${email_pattern}">${enter_email}</td>
				<td>
					<input type="text" size="50" name="email" pattern="^(\w+[\.-]?\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,6})$" required />
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td align="right" title="${pass_pattern}">${enter_password}</td>
					<td><input type="password" size="51" name="password" pattern="^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z]).{8,}$" required />
				</td>
			</tr>
			<tr>
				<td></td>
