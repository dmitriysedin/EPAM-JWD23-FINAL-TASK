...
<body>
 <fmt:setLocale value="${sessionScope.local}" />
    ...
    <fmt:message bundle="${loc}" key="locale.registration.first_name_pattern" var="first_name_pattern" />
    <fmt:message bundle="${loc}" key="locale.registration.last_name_pattern" var="last_name_pattern" />
    <fmt:message bundle="${loc}" key="locale.registration.email_pattern" var="email_pattern" />
...
<h1>${welcome_message}</h1>
	...
	<table width="100%" cellspacing="0" cellpadding="4">
		...
		<td><input type="text" size="50" name="first_name"  pattern="^([a-zA-Z]){3,44}$" required /></td>
		</tr>
		<tr>
		<td align="right" title="${last_name_pattern}">${enter_last_name}</td>
		<td><input type="text" size="50" name="last_name"  pattern="^([a-zA-Z]){3,44}$" required /></td>
		</tr>
		<tr>
		<td align="right" title="${email_pattern}">${enter_email}</td>
		<td><input type="text" size="50" name="email" pattern="^(\w+[\.-]?\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,6})$" required /></td>
		</tr>
		<tr>
		<td align="right" width="100" title="${login_pattern}">${enter_login}</td>
		<td><input type="text" size="50" name="login" pattern="^[a-zA-Z][a-zA-Z0-9-_\.]{3,20}$" required /></td>
		</tr>
		<tr>
		<td align="right" title="${pass_pattern}">${enter_password}</td>
		<td><input type="password" size="51" name="password" pattern="^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z]).{8,}$" required /></td>
		</tr>
		<tr>
		<td></td>
		<td>
		<input type="hidden" name="command" value="registration">
		<input type="submit" value="${enter_registration}" />
		</td>
		</tr>
		</table>
