...
<body>
	...
	<fmt:message bundle="${loc}" key="locale.default.go_to_home_page"
		var="home" />
	<fmt:message bundle="${loc}" key="locale.movies_info.rate_date"
		var="rate_date" />
	<fmt:message bundle="${loc}" key="locale.movies_info.rate_value"
		var="rate_value" />
	<fmt:message bundle="${loc}" key="locale.movies_info.rate_author"
		var="rate_author" />
	<fmt:message bundle="${loc}" key="locale.movies_info.rate_comment"
		var="rate_comment" />
	<fmt:message bundle="${loc}" key="locale.movies_info.rate_table_title"
		var="rate_table_title" />
	...
	<fmt:message bundle="${loc}" key="locale.default.logout_message"
		var="logout_message" />
	<fmt:message bundle="${loc}" key="locale.addNewRate.rate_added_message"
		var="added_rate_success_message" />
	<fmt:message bundle="${loc}" key="locale.addNewRate.double_add_message"
		var="double_add_rate_message" />
	...
	<table width="100%" cellspacing="0" cellpadding="4">
		...
		<tr>
			<td align="left" width="30%"><c:out value="${movie_title}" /></td>
			<td align="left" width="70%"><c:out
					value="${sessionScope.movieInfo.movieTitle}" /></td>
		</tr>
		<tr>
			<td align="left"><c:out value="${movie_director}" /></td>
			<td align="left"><c:out
					value="${sessionScope.movieInfo.movieDirector}" /></td>
		</tr>
		<tr>
			<td align="left"><c:out value="${movie_released_year}" /></td>
			<td align="left"><c:out
					value="${sessionScope.movieInfo.movieReleasedYear}" /></td>
		</tr>
		<tr>
			<td align="left"><c:out value="${movie_rate}" /></td>
			<td align="left"><c:out
					value="${sessionScope.movieInfo.movieAVGRate}" /></td>
		</tr>
			...
			<c:if
				...
				<tr>
					...
					<td align="center" width="75"><c:out
							value="${rates.rateValue}"></c:out></td>
					<td align="center" width="200"><c:out
							value="${rates.userFirstName}"></c:out> <c:out
							value="${rates.userLastName}"></c:out></td>
					<td align="center" width="100"><c:out
							value="${rates.rateComment}"></c:out></td>
				</tr>
			</c:if>
	...
	<form action="Servlet" method="post">
		<c:if test="${count > sessionScope.concreteMovieLastRow}">
			...
			<input type="hidden" name="last_row_parameter_name"
				value="concreteMovieLastRow">
			<input type="hidden" name="number_of_pages_parameter_name"
				value="concreteMovieNumberOfPages">
			<input type="hidden" name="current_page_number_parameter_name"
				value="concreteMovieNumberOfCurrentPage">
			<input type="hidden" name="current_page_URL"
				value="Servlet?command=goToShowConcreteMovieInfoPageCommand">
