...
<body>
	...
	<fmt:message bundle="${loc}" key="locale.addNewMovie.welcome_message"
		var="welcome_message" />
	...
	<fmt:message bundle="${loc}" key="locale.addNewMovie.add_movie"
		var="add_movie" />
	<fmt:message bundle="${loc}" key="locale.addNewMovie.movie_added_message"
		var="added_movie_success_message" />
	<fmt:message bundle="${loc}" key="locale.default.go_to_home_page"
		var="home" />
	<fmt:message bundle="${loc}"
		key="locale.addNewMovie.movie_released_year_pattern"
		var="movie_released_year_pattern" />
	<fmt:message bundle="${loc}" key="locale.default.go_to_home_page"
		var="home" />
	...
	<form action="Servlet" method="post">
		<table width="100%" cellspacing="0" cellpadding="4">
			...
			<tr>
				...
				<td><input type="text" size="50" name="movie_released_year"
					pattern="[0-9]{4,4}" required /></td>
			</tr>
			<tr>
			<tr>
				<td></td>
				<td><input type="hidden" name="command" value="addNewMovie">
					<input type="submit" value="${add_movie}" /></td>
			</tr>
		</table>
	</form>
	...
	<h3>
		...
		<c:out value="${added_movie_success_message}" />
		</c:if>
	</h3>
